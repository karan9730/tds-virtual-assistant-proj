...
this problem  
Image: Here's a caption for the image: **The server encountered an error
because the "query" field is required but missing from the request. The
application is running locally on port 8000, and the request was attempting to
find the status of a ticket.**
